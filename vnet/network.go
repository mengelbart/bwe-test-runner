...
func createNetwork(ctx context.Context, left, right routerConfig) (*router, *router, *vnet.Router, error) {
	...
	leftDelay, err := vnet.NewDelayFilter(leftNIC, 50*time.Millisecond)
	if err != nil {
		return nil, nil, nil, err
	}
	...
	rightDelay, err := vnet.NewDelayFilter(rightNIC, 50*time.Millisecond)
	if err != nil {
		return nil, nil, nil, err
	}
	...
	if err = wan.AddNet(rightNIC); err != nil {
		return nil, nil, nil, err
	}
